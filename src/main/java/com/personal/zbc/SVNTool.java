...
public class SVNTool {
    ...
    public static void main(String[] args) throws Exception {
        ...
        //生成路径信息set
        genSvnHistoryInfo(svnTool);

        //输出路劲信息到文件
        saveSvnCommitPathInfoToFile(svnTool);

        //拷贝class文件
        copyClassFileProcess(svnTool.getClassInfos());

        //拷贝java文件
        copyJavaFileProcess(svnTool.getJavaInfos());
    }
    ...
    private static void genSvnHistoryInfo(SVNTool svnTool) throws Exception {
        if ("0".equalsIgnoreCase(svnPro.getProperty("version.type"))) {
            //读取提交记录信息
            svnTool.searchLog(Integer.valueOf(svnPro.getProperty("start.version")), Integer.valueOf(svnPro.getProperty("end.version")));
            ...
            for (String item : Arrays.asList(svnPro.getProperty("special.version").split(","))) {
                //读取提交记录信息
                svnTool.searchLog(Integer.valueOf(item), Integer.valueOf(item));
            }
        }
    }
     ...
     * @throws IOException
     */
    private static void saveSvnCommitPathInfoToFile(SVNTool svnTool) throws IOException {
        ...
        writeFileInfo(classPath, svnTool.getClassInfos());

        writeFileInfo(javaPath, svnTool.getJavaInfos());
    }
